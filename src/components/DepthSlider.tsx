...
export const DepthSlider: React.FC = memo(() => {
  ...
  const levels: { val: DepthLevel; label: string; desc: string }[] = [
    { val: 1, label: 'Nível 1', desc: 'Removes subjective noise and social posturing.' },
    { val: 2, label: 'Nível 2', desc: 'Logical extraction. Minimum information entropy.' },
    { val: 3, label: 'Nível 3', desc: 'Etheric reconstruction. Sensory frame decoding.' },
  ];
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      <div className="flex justify-between items-end">
        ...
        <span className="text-[10px] uppercase tracking-[0.2em] font-mono text-[#a0a0a0]">Tier 0{depth}</span>
      </div>
      ...
      <div className="flex w-full pt-2 gap-1 rounded border border-[#333] bg-[#0a0a0a] p-1 overflow-hidden" role="radiogroup">
        {levels.map((l) => (
          <button
            ...
            className={cn(
              "flex-1 min-h-[48px] flex items-center justify-center text-xs font-bold uppercase transition-colors duration-100 rounded touch-manipulation px-2",
              depth === l.val 
                ? "bg-[#333] text-[#fff]" 
